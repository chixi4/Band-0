...
def draw_usage() -> Screen:
    ...
    s.pill((166, 8, 188, 18), radius=1, fill=WHITE, outline=BLACK)
    s.d.rectangle((188, 11, 190, 15), fill=BLACK)
    for i in range(4):
        s.pill((169 + i * 4, 11, 171 + i * 4, 15), radius=0, fill=BLACK)
    s.text(12, 47, "0%", TEXT)
    s.pill((122, 43, 188, 59), radius=2, fill=WHITE, outline=BLACK)
    s.text(128, 45, "Current", TEXT)
    s.progress(12, 68, 176, 0)
    s.text(12, 82, "100% left / --", TEXT)
    s.text(12, 105, "16%", TEXT)
    s.pill((122, 101, 188, 117), radius=2, fill=WHITE, outline=BLACK)
    s.text(132, 103, "Weekly", TEXT)
    s.progress(12, 126, 176, 16)
    s.text(12, 140, "84% left / 8h 5m", TEXT)
